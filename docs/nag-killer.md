...
Dashboard builds expose the built-in path as Off plus Modes A, B, and C. Mode A uses fixed positive torque, Mode B uses a bounded four-value burst/pause cycle, and Mode C requires fresh DAS hands-on and valid steering context. Modes A/B are blocked on HW4 after fixed/burst torque injection caused control faults on current vehicle software. Mode C uses DAS status `0x399` on Legacy/HW3 and `0x39B` on HW4, plus steering angle `0x129`. These controls are direct firmware settings, not plugins, and default to Off.
